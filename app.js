/* The Fine-Tuning Index — client. Fetch data.json, render forge cards, wire search/filter/sort. */
(function () {
  "use strict";
  ...
  $("#theme").addEventListener("click", function () {
    ...
    try { localStorage.setItem("ft-theme", cur); } catch (e) {}
  });
  ...
  function card(it, i) {
    ...
    return (
      '<a class="card" href="/p/' + esc(it.slug) + '/" style="transition-delay:' + (i % 12) * 32 + 'ms">' +
        '<div class="card-body"><div class="card-top"><span class="rank disp">' + String(it.rank).padStart(2, "0") + "</span>" +
  ...
  function render() {
    ...
    if (!list.length) { grid.innerHTML = '<div class="empty">No fine-tuning tools match. Try a broader search.</div>'; return; }
